#include <Arduino.h>
#include <freertos/FreeRTOS.h>
#include <freertos/task.h>

#define PIN 26

uint8_t val=0;
bool ctrl=0;
char opcao;
int incomingByte, incomingNumber;

uint8_t  sine_wave[256] = {
  
};

uint16_t  pwmP[10] = {  0xC8, 0x190, 0x258, 0x320, 0x3E8, 0x4B0, 0x578, 0x640, 0x708, 0x7D0 };
uint16_t  pwmN[10] = { 0x708, 0x640, 0x578, 0x4B0, 0x3E8, 0x320, 0x258, 0x190,  0xC8,  0x00 };

unsigned long NextMillis;

void Senoidal();
void Triangular();
void DenteSerra();
void Quadrada(uint16_t pwmP, uint16_t pwmN);
